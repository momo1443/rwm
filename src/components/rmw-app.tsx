  ...
  return (
    <>
      ...
      <main className="desktop-app min-h-screen">
        {screen === "landing" && <Landing key={`${testMode}-${testMode ? condition : "auto"}`} locale={locale} setLocale={setLocale} participantId={participantId} testMode={testMode} initialCondition={testMode ? condition : null} startError={startError} onStart={async (chosenCondition) => {
          if (startingRef.current) return;
          startingRef.current = true;
          setStartError("");
          try {
            const sessionId = beginStudySession();
            const requestedCondition: Condition = chosenCondition || condition;
            ...
            eventLog("research_task_started", { taskId: assignedTaskId, assignment: assignmentMode === "manual" ? "manual_test" : "balanced_three_condition", protocolVersion: "reasoning-recovery-v4-matched-preinterruption", participantId, condition: assignedCondition }, { stage: "task_setup" });
            setScreen("brief");
          } finally {
...
function Landing({
  ...
  startError: string;
  onStart: (condition: Condition | null) => Promise<void>;
    ...
    <section className="mx-auto grid max-w-6xl grid-cols-[1.08fr_.92fr] items-center gap-16 px-8 py-20">
      ...
      <div className="rounded-2xl border bg-white/90 p-8 shadow-[0_24px_70px_rgba(34,42,70,.10)] backdrop-blur">
        ...
        {testMode ? <fieldset className="mt-7">
          ...
          <p className="mt-1 text-xs leading-5 text-muted-foreground">{locale==="zh-CN"?"研究者测试模式：手动选择一种恢复方式。":"Researcher test mode: manually choose a recovery method."}</p>
        ...
        </fieldset> : <p className="mt-5 text-xs leading-5 text-muted-foreground">{locale==="zh-CN"?"所有参与者完成相同任务与中断；系统会随机分配一种中断后恢复方式。":"All participants complete the same task and interruption; one post-interruption recovery method is assigned at random."}</p>}
        ...
        <TimedButton seconds={5} ready={consent&&Boolean(participantId)&&(!testMode||Boolean(conditionChoice))&&!submitting} locale={locale} label={t.enter} blockedLabel={submitting?(locale==="zh-CN"?"正在创建本次运行…":"Setting up your session…"):testMode&&!conditionChoice?(locale==="zh-CN"?"请先选择实验方式":"Choose a method first"):(locale==="zh-CN"?"请勾选同意":"Provide consent to continue")} onClick={()=>{if((testMode&&!conditionChoice)||submitting)return;setSubmitting(true);onStart(conditionChoice).finally(()=>setSubmitting(false));}} className="mt-7 h-12 w-full" />
