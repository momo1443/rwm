...
export function RecoveryProbePage({ locale, stage, assessment, onSubmit }: { locale: Locale; stage: RecoveryProbeStage; assessment: RecoveryAssessment; onSubmit: (probe: RecoveryProbe) => void }) {
  ...
  return <main className="min-h-screen bg-[#f7f6f2] px-6 py-10">
    <div className="mx-auto max-w-4xl rounded-2xl border bg-white p-7 shadow-[0_18px_60px_rgba(35,40,65,.08)]">
      <div className="flex items-start justify-between gap-4"><div><p className="font-mono text-[10px] uppercase tracking-[.16em] text-primary">{text.eyebrow}</p><h1 className="mt-2 text-2xl font-semibold tracking-tight">{text.title}</h1><p className="mt-2 text-sm leading-6 text-muted-foreground">{text.description}</p></div><Brain size={30} className="text-primary" /></div>
