  ...
  return <main className="min-h-screen bg-[#f7f6f2] p-5 lg:p-8">
    <div className="mx-auto max-w-[1500px]"><header className="mb-5 flex items-end justify-between"><div><p className="text-xs uppercase tracking-widest text-muted-foreground">Recovery outcome rubric v2</p><h1 className="mt-1 text-2xl font-semibold">结果盲评</h1><p className="mt-2 text-sm text-muted-foreground">提供任务题干以判断相关性；仍不显示恢复条件、参与者编号或恢复支持内容。</p></div><p className="text-sm text-muted-foreground">已评 {results.filter((result) => result.blind_reviewed_at).length} / {results.length}</p></header>
      {results.length === 0 ? <div className="rounded-xl border bg-white p-8 text-center text-muted-foreground">暂无同时具有中断前快照和最终 memo 的已完成记录。</div> : <div className="grid gap-5 xl:grid-cols-[210px_1fr_360px]">
        ...
        <section className="min-h-[680px]"><div className="mb-4 rounded-xl border border-blue-200 bg-blue-50 p-4"><p className="text-xs font-semibold text-blue-900">{active?.task_label}</p><p className="mt-2 text-sm leading-6 text-blue-950">{active?.task_question}</p><p className="mt-2 text-[11px] text-blue-800">任务对评审可见；恢复条件与具体支持材料保持隐藏。</p></div><div className="grid gap-4 lg:grid-cols-2"><MemoCard title="中断前 memo（冻结快照）" text={active?.phase_one_memo || ""}/><MemoCard title="最终 memo（恢复阶段结束）" text={active?.final_memo || ""}/></div></section>
