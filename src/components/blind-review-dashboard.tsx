...
function RubricPanel({
  ...
  return <div className="rounded-xl border bg-white p-5">
    ...
    <div className="mt-5 border-t pt-4"><p className="text-sm font-medium">{beforeLabel} {scoreTotal(beforeScores)} / 20 · {afterLabel} {scoreTotal(afterScores)} / 20</p><p className="mt-1 text-sm font-semibold text-primary">质量变化 {scoreTotal(afterScores) - scoreTotal(beforeScores) >= 0 ? "+" : ""}{scoreTotal(afterScores) - scoreTotal(beforeScores)}</p></div>
  </div>;
}
